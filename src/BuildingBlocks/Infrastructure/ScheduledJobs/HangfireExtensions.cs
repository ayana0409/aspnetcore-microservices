...
namespace Infrastructure.ScheduledJobs
{
    public static class HangfireExtensions
    {
        ...
        {
            var settings = services.GetOption<HangFireSettings?>("HangFireSettings");
            if (settings == null || settings.Storage == null 
                || string.IsNullOrEmpty(settings.Storage.ConnectionString))
                throw new ArgumentNullException(nameof(HangFireSettings), "HangfireSettings is not configured properly!");
        ...
        private static IServiceCollection ConfigureHangfireServices(this IServiceCollection services, HangFireSettings settings)
        {
            ...
            switch (settings.Storage.DbProvider.ToLower())
            {
                case "mongodb":
                    ...
                    services.AddHangfire((privider, config) =>
                    {
                    ...
                    break;
                case "postgresql":
                    services.AddHangfire(x => x.UsePostgreSqlStorage(settings.Storage.ConnectionString));
                    break;
                case "mssql":
                    break;
